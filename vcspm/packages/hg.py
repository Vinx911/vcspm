...
class HgPackage(Package):

    def __init__(self, name, info):
        super().__init__(name, info)
        self.url = info.get('url', None)
        self.hg_url = info.get('hg', None)
        self.revision = info.get('revision', None)

        self.hg_url = self.hg_url or self.url

        if self.hg_url is None:
            raise

    def _clone_hg_repository(self, local_path):
        """
        从hg仓库下载包
        """
        target_path = utils.escapify_path(os.path.join(local_path, self.name))
        target_exists = os.path.exists(target_path)
        logging.info("克隆 {} 到 {}".format(self.hg_url, target_path))
        ...
        if not repo_exists:
            ...
            utils.die_if_non_zero(
                utils.exec_shell("{} clone {} {}".format(SHELL_HG, self.hg_url, target_path))
            )
        else:
            logging.info("仓库 {} 已经存在，将进行拉取".format(target_path))
            utils.die_if_non_zero(
                utils.exec_shell("{} pull -R {}".format(SHELL_HG, target_path))
            )

        if self.revision is None:
            self.revision = "HEAD"
        utils.die_if_non_zero(
            utils.exec_shell("{} update -R {} -C {}".format(SHELL_HG, target_path, self.revision))
        )
        utils.die_if_non_zero(
            utils.exec_shell("{} purge -R {}  --config extensions.purge=".format(SHELL_HG, target_path))
        )
    ...
    def download(self):
        ...
        repo_path = self._clone_hg_repository(cache_path)
